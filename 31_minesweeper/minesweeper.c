...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  // A. malloc and initialize a board_t
  // B. fill Board.board with UNKNOWN and call addRandomMine to put numMines on Board.board
  //  B:notes: 1. all fields of board_t Board should be initialized at this point
  //           2. change the parameters of addRandomMine
  // notes: 1. board[y][x] means the y row, and x collumn,
  //        where 0 <= y < height and 0 <= x < width.
  
  // A

  //  malloc enough space for a  board_t
  //   first assert that h, w have not the null case
  assert(h > 0 && w > 0);
  
  //   malloc for a board_t
  board_t *b = malloc(sizeof(*b));
  if (!b) {
    fprintf(stderr, "error allocating space in the HEAP");
    exit(EXIT_FAILURE);
  }
  //   then malloc for b->board
  b->board = malloc(h*sizeof(int *));
  if ( !(b->board) ) {
    fprintf(stderr, "error allocating space in the HEAP");
    exit(EXIT_FAILURE);
  }
  //   then malloc for each row b->board[y]
  ...
  //   if all allocation was succesful initializa the rest *b board_t
  ...
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      if (y+i >= 0 && y+i < b->height && x+j >= 0 && x+j < b->width) {
        near_mines += IS_MINE( b->board[y+i][x+j] );
      }
    }
  }
  return near_mines;
...
void freeBoard(board_t * b) {
  ...
  // The memory has to be freed backwards!
  // First the blocks allocated to board[y] 0<=y<h
  // Second the block allocated to board
  // Last the block allocated to b 
  //
  ...
  //                    | 
  //                    |                    |                |   |               |   
  // b -> .board    -> [0] [1].. [height] -> |[0][0].. [0][w] |.. |[h][0].. [h][w]|  
  //      .height
  //      .witdh
  //      .totalMines
